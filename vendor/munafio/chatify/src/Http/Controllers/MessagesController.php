...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if($request->hasFile('file'))
        {
            ...
            if($file->getSize() < 150000000)
            {
                ...
                {
                    $error_msg = __("File extension not allowed!");
                }
            }
            else
            {
                $error_msg = __("File size is too long!");
            }
        }
    ...
    public function fetch(Request $request)
    {
        ...
        return Response::json(
            [
                'count' => $query->count(),
                'messages' => '<p class="message-hint"><span>' . __("Say 'hi' and start messaging") . '</span></p>',
            ]
        );
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        )->where('messages.from_id', Auth::user()->id)->orWhere('messages.to_id', Auth::user()->id)->orderBy('messages.created_at', 'desc')->get()->unique('id');
        ...
        {
            $members = User::where('created_by', '=', $objUser->creatorId())->where('type', '!=', 'client')->get();
        }
        else
        {
            $members = User::where('created_by', '=', $objUser->creatorId())->where('id', '!=', $objUser->id)->where('type', '!=', 'client')->orWhere('id', '=', $objUser->creatorId())->get();
        }
    ...
    public function search(Request $request)
    {
        ...
        if($objUser->type == 'company')
        {
            $records = User::where('created_by', '=', $objUser->creatorId())->where('type', '!=', 'client')->where('name', 'LIKE', "%{$input}%");
        }
        else
        {
            $records = User::where('created_by', '=', $objUser->creatorId())->where('id', '!=', $objUser->id)->where('type', '!=', 'client')->orWhere('id', '=', $objUser->creatorId())->where('name', 'LIKE', "%{$input}%");
        }
        ...
        return Response::json(
            [
                'records' => $records->count() > 0 ? $getRecords : '<p class="message-hint"><span>' . __('Nothing to show.') . '</span></p>',
                'addData' => 'html',
            ], 200
        );
    }
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        return Response::json(
            [
                'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>' . __('Nothing shared yet') . '</span></p>',
            ], 200
        );
    }
    ...
    public function getMessagePopup()
    {
        ...
        $messages = Message::whereIn(
            ...
            $query->select(\DB::raw('MAX(id)'))->from('messages')->where('to_id', '=', $user->id)->where('seen', '=', 0);
        }
        )->orderBy('id', 'desc')->get();
