...
class ProjectstagesController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        if(\Auth::user()->can('edit project stage'))
        {
            $project_stages = Projectstages::findOrfail($id);
            if($project_stages->created_by == \Auth::user()->creatorId())
            {
                $validator = \Validator::make(
                    $request->all(), [
                                       'name' => 'required|max:20',
                ...
                $project_stages->name  = $request->name;
                $project_stages->color = '#' . $request->color;
                $project_stages->save();
