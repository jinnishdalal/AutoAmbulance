    ...
    @can('create estimation')
        <div class="all-button-box row d-flex justify-content-end">
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('estimations.create') }}" data-ajax-popup="true" data-title="{{__('Create Estimate')}}" class="btn btn-xs btn-white btn-icon-only width-auto"><i class="fas fa-plus"></i> {{__('Create')}} </a>
            </div>
        </div>
    @endcan
@endsection
                        ...
                        <table class="table table-striped dataTable">
                            ...
                            @foreach ($estimations as $estimate)
                                <tr>
                                    <td class="Id">
                                        @can('view estimation')
                                            <a href="{{route('estimations.show',$estimate->id)}}"> <i class="fas fa-file-estimate"></i> {{ Utility::estimateNumberFormat($estimate->estimation_id) }}</a>
                                        @else
                                            {{ Utility::estimateNumberFormat($estimate->estimation_id) }}
                                        @endcan
                                    </td>
                                    ...
                                    @if(Auth::user()->type != 'client')
                                        <td class="Action">
                                            ...
                                            @can('View Estimation')
                                                ...
                                                @endcan
                                                @can('edit estimation')
                                                    <a href="#" data-url="{{ URL::to('estimations/'.$estimate->id.'/edit') }}" data-ajax-popup="true" data-title="{{__('Edit Estimation')}}" class="edit-icon" ><i class="fas fa-pencil-alt"></i></a>
                                                @endcan
                                                @can('delete estimation')
                                                    <a href="#" class="delete-icon"  data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$estimate->id}}').submit();"><i class="fas fa-trash"></i></a>
